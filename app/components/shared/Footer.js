...
const Footer = () => {
  const iconItems = [
    { icon: faEnvelope , url: 'mailto:someone@example.com', text: 'sales@cofocsp.com' , name: 'Email' },
    { icon: faPhone, url: 'tel:+1234567890', text: '+97145777890', name: 'Call' },
    { icon: faMapMarkerAlt, url: 'https://www.google.com/maps', text: ' Floor 29, Office# 32 - Al Saqr Business Tower, Sheikh Zayed Road, Dubai, United Arab Emirates' , name: 'Location' },
    ...
    <footer className="Footer">
      ...
      <section className='Bottom'>
        <section className='First'>
          <img src='' />
          <p>Maids table how learn drift but purse stand yet set. Music me house could among oh as their.</p>
        </section>
        ...
        {socialMediaLinks.map((link, index) => (
          <li key={index} className="social-media-item">
            <a href={link.url} target="_blank" rel="noopener noreferrer" aria-label={link.name}>
              <FontAwesomeIcon icon={link.icon} className="social-media-icon" />
            </a>
          </li>
        ))}
      </ul>
      <p className="text-sm">&copy; {new Date().getFullYear()} Created by Company Formation</p>
         </section>
      </section>
  </footer>
  )
